...
class Ui_MainWindow:
    """
    Pagrindinė vartotojo sąsajos generavimo klasė.
    Atsakinga tik už UI elementų (mygtukų, išskleidžiamųjų meniu, grafikų) instancijavimą 
    ir jų išdėstymą (layout) ekrane, nenaudojant QT Designer.
    """
    def setup_ui(self, MainWindow):
        MainWindow.setWindowTitle("Automatizuota Matavimų Sistema")
        MainWindow.resize(1450, 950)

        # Pagrindinis konteineris ir horizontalus išdėstymas
        central_widget = QWidget()
        MainWindow.setCentralWidget(central_widget)
        main_layout = QHBoxLayout(central_widget)
        main_layout.setContentsMargins(5, 5, 5, 5)
        
        # Dinaminis lango padalinimas (Splitter) leidžia vartotojui keisti skydelių pločius
        self.splitter = QSplitter(Qt.Orientation.Horizontal)
        main_layout.addWidget(self.splitter)

        # ---------------------------------------------------------
        # --- KAIRĖ PUSĖ: Aparatūros konfigūravimas ir valdymas ---
        # ---------------------------------------------------------
        ...
        # --- 2. Instrumentų valdymo skirtukai (Tabs) ---
        self.left_tabs = QTabWidget()

        # [TAB] Generatorius (Siglent SDG)
        ...
        # Apjungiamas dažnio įvedimo laukas su matavimo vienetų pasirinkimu
        self.f_widget = QWidget()
        l_f = QHBoxLayout(self.f_widget)
        l_f.setContentsMargins(0,0,0,0)
        l_f.addWidget(self.freq_in)
        l_f.addWidget(self.freq_unit)
        ...
        self.btn_apply_gen = QPushButton("Taikyti Parametrus")
        self.btn_apply_gen.setStyleSheet(STYLE_SUCCESS)
        
        self.btn_eqphase = QPushButton("Sync Fazę")
        self.btn_eqphase.setStyleSheet(STYLE_NORMAL)
        
        self.btn_gen_ch1 = QPushButton("CH1 Išvestis")
        self.btn_gen_ch1.setCheckable(True)
        self.btn_gen_ch1.setStyleSheet(STYLE_NORMAL)
        
        self.btn_gen_ch2 = QPushButton("CH2 Išvestis")
        self.btn_gen_ch2.setCheckable(True)
        self.btn_gen_ch2.setStyleSheet(STYLE_NORMAL)

        gen_l.addLayout(grid_g)
        gen_l.addWidget(self.btn_apply_gen)
        gen_l.addWidget(self.btn_eqphase)
        
        h_gen = QHBoxLayout()
        h_gen.addWidget(self.btn_gen_ch1)
        h_gen.addWidget(self.btn_gen_ch2)
        gen_l.addLayout(h_gen)
        
        gen_l.addStretch()
        self.left_tabs.addTab(tab_gen, "Gen")

        # [TAB] Oscilografas (Rigol MSO)
        tab_osc = QWidget(); osc_l = QVBoxLayout(tab_osc); osc_l.setContentsMargins(5, 5, 5, 5)
        
        self.btn_auto = QPushButton("Auto Scale")
        self.btn_auto.setStyleSheet(STYLE_PRIMARY)
        
        self.btn_run = QPushButton("STOP (Sustabdyta)")
        self.btn_run.setCheckable(True)
        self.btn_run.setStyleSheet(STYLE_DANGER)
        ...
        self.combo_meas_ch = QComboBox()
        self.combo_meas_ch.addItems(["CH1", "CH2", "CH3", "CH4"])
        
        self.btn_meas_all = QPushButton("Nuskaityti Visus Parametrus")
        self.btn_meas_all.setStyleSheet(STYLE_PRIMARY)
        
        # Oscilografo parametrų atvaizdavimo lentelė (18 eilučių, 2 stulpeliai)
        ...
        self.btn_osc_screenshot = QPushButton("Išsaugoti Ekrano Kopiją")
        self.btn_osc_screenshot.setStyleSheet(STYLE_NORMAL)
        
        self.btn_copy_meas = QPushButton("Kopijuoti Matavimus")
        self.btn_copy_meas.setStyleSheet(STYLE_NORMAL)
        ...
        # [TAB] Multimetras TTi 1604
        tab_tti = QWidget(); tti_l = QVBoxLayout(tab_tti); tti_l.setContentsMargins(5, 5, 5, 5)
        
        # Virtualaus LCD ekrano inicializacija
        self.lbl_tti_val = QLabel("---")
        self.lbl_tti_val.setStyleSheet(STYLE_LCD_DC)
        ...
        self.btn_tti_refresh = QPushButton("Nuskaityti TTi")
        self.btn_tti_refresh.setStyleSheet(STYLE_PRIMARY)
        ...
        # [TAB] Automatinis skenavimas (Bode)
        ...
        self.btn_start_bode = QPushButton("Pradėti Bode")
        self.btn_start_bode.setStyleSheet(STYLE_SUCCESS)
        
        self.btn_stop_bode = QPushButton("Stabdyti")
        self.btn_stop_bode.setStyleSheet(STYLE_DANGER)
        
        self.bode_progress = QProgressBar(); self.bode_progress.setValue(0)
        
        bode_l.addWidget(self.btn_start_bode, 7, 0, 1, 2)
        bode_l.addWidget(self.btn_stop_bode, 8, 0, 1, 2)
        bode_l.addWidget(self.bode_progress, 9, 0, 1, 2)
        bode_l.setRowStretch(10, 1)
        
        self.left_tabs.addTab(tab_bode, "Bode")

        # [TAB] Ilgalaikis registravimas (Logger)
        ...
        self.btn_start_log.setStyleSheet(STYLE_SUCCESS)
        
        self.btn_stop_log = QPushButton("Stabdyti")
        self.btn_stop_log.setStyleSheet(STYLE_DANGER)
        
        log_l.addWidget(self.lbl_log_current, 4, 0, 1, 2)
        log_l.addWidget(self.btn_start_log, 5, 0, 1, 2)
        log_l.addWidget(self.btn_stop_log, 6, 0, 1, 2)
        log_l.setRowStretch(7, 1)
        ...
        # -------------------------------------------------------------
        # --- DEŠINĖ PUSĖ: Grafikai (PyQtGraph) ir Sistemos Žurnalas ---
        # -------------------------------------------------------------
        ...
        # --- 1. Oscilograma (Realaus laiko signalų atvaizdavimas) ---
        ...
        self.btn_calc_fft = QPushButton("Skaičiuoti FFT")
        self.btn_calc_fft.setStyleSheet(STYLE_PRIMARY)
        ...
        l_g_osc.addWidget(self.graph_widget)
        l_g_osc.addLayout(h_ctrl_osc)
        self.graph_tabs.addTab(tab_g_osc, "Oscilograma")

        # --- 2. Bode Analizė (Amplitudės ir Dažnio charakteristika) ---
        ...
        self.bode_graph.setLogMode(x=True, y=False) # Logaritminė X ašis (Dažnis)
        self.bode_graph.setLabel('left', 'Stiprinimas', units='dB')
        self.bode_graph.setLabel('bottom', 'Dažnis', units='Hz')
        
        self.btn_export_bode = QPushButton("Eksportuoti Bode")
        self.btn_export_bode.setStyleSheet(STYLE_EXPORT)
        
        l_g_bode.addWidget(self.bode_graph)
        l_g_bode.addWidget(self.btn_export_bode)
        self.graph_tabs.addTab(tab_g_bode, "Bode Analizė")

        # --- 3. Ilgalaikis Registravimas (Logger) ---
        ...
        # --- 4. FFT (Greitoji Furjė Transformacija) ---
        ...
        l_g_fft.addWidget(self.fft_graph)
        l_g_fft.addWidget(self.lbl_fft_peak)
        self.graph_tabs.addTab(tab_g_fft, "FFT")

        # --- Sistemos Žurnalas (Console) ---
        ...
        # Dešinės pusės struktūros apjungimas
        ...
        # Pradinis lango skydelių pločio santykis (Valdymas : Grafikai)
